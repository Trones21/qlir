...
def test_add_running_true_inplace():
    df = pd.DataFrame({"c": [True, False, True]}).astype({"c": "boolean"})
    out = add_running_true(df, "c", inplace=True)
    assert out is df
...
def test_bars_since_true_inplace():
    ...
    out = add_bars_since_true(df, "c", inplace=True)
    assert out is df
