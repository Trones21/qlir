...
def test_custom_name_and_inplace():
    df = _ohlc_df()
    out = with_higher_close(df, name="hc_up", inplace=True)
    assert out is df
...
def test_range_expansion_highlow_and_tr():
    ...
    # highlow method
    out1 = with_range_expansion_vs_prev(df.copy(), method="highlow")
    ...
    # tr method
    out2 = with_range_expansion_vs_prev(df.copy(), method="tr")
    col2 = "range_expansion__tr"
    # recompute TR and compare with shift
    out_tr = with_true_range(df.copy())
