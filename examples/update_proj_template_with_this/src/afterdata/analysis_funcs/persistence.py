...
def persistence_analysis(df: pd.DataFrame, trendline_col: str):
    ...
    df["dir_col_up"] = df[direction].map({1: True}).astype("boolean").fillna(False)

    for_up_pers_df, _ = persistence_analysis_prep_up(df, condition_col="dir_col_up", trendline_col=trendline_col)
    ...
    df["dir_col_down"] = df[direction].map({-1:True}).astype("boolean").fillna(False)

    for_down_pers_df, _ = persistence_analysis_prep_down(df, condition_col="dir_col_down", trendline_col=trendline_col)
...
def persistence_analysis_prep_up(df: pd.DataFrame, condition_col: str, trendline_col: str) -> tuple[pd.DataFrame, list[str]]:
    '''
    ...
    df, grp_ids_sma_up_legs_col = assign_condition_group_id(df=df, condition_col=condition_col, group_col=f"{trendline_col}_up_leg_id")
...
def persistence_analysis_prep_down(df: pd.DataFrame, condition_col: str, trendline_col: str) -> tuple[pd.DataFrame, list[str]]:
    '''
    Prep for the persistence analysis func
    assign group ids 
    add a counter for each group 
    ...
    # Add group id - will need for bucketizing/summarization 
    df, grp_ids_legs_col = assign_condition_group_id(df=df, condition_col=condition_col, group_col=f"{trendline_col}_down_leg_id")
