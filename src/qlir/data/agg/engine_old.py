...
def iter_raw_ok_slices(raw_manifest: dict[str, Any]) -> Iterable[RawSliceRef]:
    ...
    log.info(f"Slices in Manifest: {len(slices)}")

    for _key, entry in slices.items():
        if not isinstance(entry, dict):
            continue
        try:
            status = entry[SLICE_STATUS_KEY] 
        except KeyError as ke:
            log.debug(ke)
            log.debug(f"Keys: {entry.keys()}")
            log.info(f"Slice excluded, could not get '{SLICE_STATUS_KEY}'")
            continue
        if entry.get("slice_status") != SLICE_OK:
            log.info(f"Slice excluded because slice status is {status}")
            continue
