...
def load_or_create_manifest(
    ...
    log.info("Creating fresh manifest (no manifest found at: %s)", manifest_path)
    return {
        "endpoint": ENDPOINT,
        "symbol": symbol,
        "interval": interval,
...
def write_full_manifest_snapshot(snapshot_dir: Path, manifest: dict, reason: str) -> None:
    """Written by the FETCHER for the Manifest Builder to consume."""
    ...
    if reason:
        log.info("Full manifest snapshot taken | path=%s reason=%s", final, reason)
...
def write_manifest_snapshot(manifest_path: Path, manifest: Dict) -> None:
    """Written by the MANIFEST BUILDER (sole writer of manifest.json)."""
    _write_manifest_atomic(manifest_path, manifest)


def seed_manifest_with_expected_slices(manifest, expected_slices: list[SliceKey]):
    """Ensure every expected slice exists in the manifest with at least 'missing' status."""
    changed = False
...
def update_manifest_with_classification(manifest, classified: SliceClassification):
    """Mutate manifest in-place using classification results."""
    for slice_key in classified.missing:
        manifest["slices"][slice_key.canonical_slice_composite_key()]["slice_status"] = SliceStatus.MISSING
    for slice_key in classified.partial:
        manifest["slices"][slice_key.canonical_slice_composite_key()]["slice_status"] = SliceStatus.PARTIAL
    for slice_key in classified.needs_refresh:
        manifest["slices"][slice_key.canonical_slice_composite_key()]["slice_status"] = SliceStatus.NEEDS_REFRESH
    for slice_key in classified.complete:
        manifest["slices"][slice_key.canonical_slice_composite_key()]["slice_status"] = SliceStatus.COMPLETE
    for slice_key in classified.failed:
        manifest["slices"][slice_key.canonical_slice_composite_key()]["slice_status"] = SliceStatus.FAILED
