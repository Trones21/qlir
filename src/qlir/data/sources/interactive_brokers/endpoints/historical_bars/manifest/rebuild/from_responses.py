...
from qlir.data.sources.interactive_brokers.endpoints.historical_bars.manifest.read_metadata import (
    read_response_metadata,
)
from qlir.data.sources.interactive_brokers.endpoints.historical_bars.manifest.summary import (
    update_summary,
)
...
def rebuild_manifest_from_responses(
    *,
    ...
    """Rebuild the in-memory manifest by scanning responses/ (crash recovery / cold start)."""
    log.warning("Rebuilding manifest from filesystem responses (this may take time)")
    ...
    log.info("Manifest rebuild complete | total=%d", manifest["summary"]["total_slices"])
    return manifest
