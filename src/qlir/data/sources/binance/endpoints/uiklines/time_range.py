...
from .urls import build_UIKline_url, interval_to_ms
...
def probe_earliest_open_time(
    ...
    """
    Probe Binance for the earliest available UIKline openTime for a symbol+interval.

    Strategy:
      - Request 1 candle starting from Unix epoch (startTime=0, limit=1).
      - If Binance has data, it should return the earliest UIKline >= 0.
    ...
    # UIKline format: [ openTime, open, high, low, close, volume, closeTime, ... ]
    return int(data[0][0])
...
def probe_latest_open_time(
    ...
    """
    Probe Binance for the latest available UIKline openTime for a symbol+interval.
...
def compute_time_range(
    ...
    Semantics (for now):
      ...
      - start_ms:
            Earliest available UIKline openTime from Binance
            (based on a probe at startTime=0, limit=1).

      - end_ms:
            Usually "current" latest UIKline openTime (based on a probe with
      ...
      - Slice alignment:
            ...
            (generate_UIKline_slices) will partition this into spans of
    ...
    if earliest is None:
        ...
        raise RuntimeError(f"No UIKline data found for {symbol} @ {interval}")
