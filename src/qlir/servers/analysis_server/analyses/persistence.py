...
def persistence_analysis(df: pd.DataFrame, trendline_col: str) -> list[NamedDF]:
    ...
    for_up_pers_adf = persistence_up_legs(df, direction_col, trendline_col)
    up_legs_dists = condition_persistence(for_up_pers_adf.df, f"{trendline_col}_up_leg_id", "up_leg_run_len")
    ...
    for_down_pers_adf = persistence_down_legs(df, direction_col, trendline_col)
    down_legs_dists = condition_persistence(for_down_pers_adf.df, f"{trendline_col}_down_leg_id", "down_leg_run_len")
