...
def flag_relations(
    ...
    *,
    ...
    if touch_min_abs > 0:
        tol = np.maximum(tol, touch_min_abs)

    rel = np.where(diff.abs() <= tol, "touch", np.where(diff > 0, "above", "below"))
    out[out_rel] = pd.Categorical(rel, categories=["below", "touch", "above"], ordered=True)
